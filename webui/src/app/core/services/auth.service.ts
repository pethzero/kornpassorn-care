...
export class AuthService {
  ...
  constructor(private http: HttpClient) {
    const token = localStorage.getItem('token');
    if (token) {
      // OPTIONAL: เรียก API /me เพื่อดึง user จาก token
      this.http.get<User>('${environment.apiUrl}/profile', {
        ...
        next: (user) => this.currentUserSubject.next(user),
        error: () => this.logout() // token invalid ก็ล้าง
      });
    }
  }
  ...
  loginAsGuest(): Observable<any> {
    return this.http.post<{ access_token: string }>('{environment.apiUrl}/auth/guest', {}).pipe(
      tap(response => {
        const token = response.access_token;
        localStorage.setItem('token', token);
